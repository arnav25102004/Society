...
describe('visitors router — resident approve (JWT auth only, no HMAC)', () => {
  // The per-user Redis-backed HMAC signing key was intermittently rejecting
  // legitimate same-device requests with "Invalid request signature" in production —
  // removed in favor of the same protection level /reject already used successfully.
  const app = buildApp();
  beforeEach(() => jest.resetAllMocks());

  it('approves the visitor for the matching resident with just a valid JWT', async () => {
  ...
  it('rejects approve for a resident of a different flat', async () => {
    const encryptedFlat = encryptSearchable('A-101');
    visitorFindUnique.mockResolvedValueOnce({
      id: 'visitor-1', status: 'pending', societyId: SOCIETY_ID, flatNumber: encryptedFlat,
    });
    societyMemberFindFirst.mockResolvedValueOnce(null); // no membership matches this flat
    ...
    expect(res.status).toBe(403);
    expect(res.body.message).toMatch(/not your flat/i);
  });

  it('rejects approve without a valid JWT', async () => {
    const res = await request(app).put('/visitors/visitor-1/approve');
