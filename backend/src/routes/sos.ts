...
sosRouter.post('/', validate(triggerSosSchema), async (req: Request, res: Response) => {
  ...
  // Notify guards, committee, and nearby residents
  const urgentMembers = await prisma.societyMember.findMany({
    where: {
      societyId,
      role: { in: ['guard', 'committee', 'admin'] },
      status: 'approved',
    },
...
sosRouter.put('/:id/resolve', validate(resolveSchema), async (req: Request, res: Response) => {
  ...
  // Notify the resident who raised the SOS
  const raiser = await prisma.user.findUnique({
    where: { id: alert.userId },
    select: { expoPushToken: true },
  });
  if (raiser?.expoPushToken) {
    notificationService.sosResolved({
      token: raiser.expoPushToken,
      alertId: alert.id,
    });
  }
